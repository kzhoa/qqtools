...
def model_wrapper(model):
    ...
    def enegry_fn(inpt):
        (species, coordinates) = inpt
        batch = torch.zeros(species)
        return model(species, coordinates, batch)
...
class Calculator(ase.calculators.calculator.Calculator):
    ...
    def calculate(self, atoms=None, properties=["energy"], system_changes=ase.calculators.calculator.all_changes):
        ...
        if pbc_enabled:
            if "stress" in properties:
                cell = cell @ scaling
            energy = self.model((species, coordinates), cell=cell, pbc=pbc).energy
        else:
            energy = self.model((species, coordinates)).energy
