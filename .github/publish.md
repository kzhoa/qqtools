...
3. From that clean candidate commit, run
   ...
   lifecycle for that target, then runs the complete qexp source-level regression suite.
...
8. The tag workflow trusts the completed local preflight. It validates the tag/version/changelog
   relationship, rebuilds the exact tagged wheel, runs the only installed-wheel E2E check, and only
   then creates the GitHub Release and publishes to PyPI. On E2E failure, it uploads the JUnit
   report and retained runtime evidence for seven days; it does not repeat the compatibility
   lifecycle gate.
...
- Release validation runs the complete qexp unit and integration suites locally, then validates the
  exact tagged, installed wheel in the publish workflow. Installed-wheel E2E does not replace the
