...
def test_dispatch_claims_and_launches_with_reservation(tmp_path: Path):
    ...
    assert run_dispatch_cycle(cfg, available_gpus=[0], executor=executor) == [
        next(iter((cfg.shared_root / "tasks").glob("*.json"))).stem
    ]
...
def test_prelaunch_cancel_with_missing_attempt_still_clears_claim_and_reservation(tmp_path: Path):
    cfg = init_shared_root(tmp_path / ".qexp", "gpu-1", runtime_root=tmp_path / "rt")
    task = submit(cfg, ["echo", "ok"])
    attempt = claim_task(cfg, task.task_id, [0])
    assert attempt is not None
...
def test_prelaunch_cancel_increments_revision_once_and_event_matches_final_task(tmp_path: Path, monkeypatch):
    cfg = init_shared_root(tmp_path / ".qexp", "gpu-1", runtime_root=tmp_path / "rt")
    task = submit(cfg, ["echo", "ok"])
    attempt = claim_task(cfg, task.task_id, [0])
    assert attempt is not None
...
def test_blocked_orphan_retry_supersedes_orphan_and_records_audit_event(tmp_path: Path):
    ...
    current = AttemptRecord.from_dict(read_json(attempt_path(cfg.shared_root, task.task_id, attempt.attempt_number)))
...
def test_agent_acknowledges_termination_only_after_process_disappears(tmp_path: Path, monkeypatch):
    cfg = init_shared_root(tmp_path / ".qexp", "gpu-1", runtime_root=tmp_path / "rt")
    task = submit(cfg, ["echo", "ok"])
    attempt = claim_task(cfg, task.task_id, [0])
    assert attempt is not None
...
def test_agent_finalizes_missing_recovered_process_and_releases_gpu(tmp_path: Path, monkeypatch):
    cfg = init_shared_root(tmp_path / ".qexp", "gpu-1", runtime_root=tmp_path / "rt")
    task = submit(cfg, ["echo", "ok"])
    attempt = claim_task(cfg, task.task_id, [0])
    assert attempt is not None
