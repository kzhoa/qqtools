...
from qqtools.plugins.qexp import cli, init_shared_root, submit
...
def test_watch_rejects_redirected_stdout_before_project_resolution(capsys) -> None:
    assert cli.main(["task", "show", "task-1", "--watch"]) == 2
...
def test_continuous_cli_routes_defaults_and_explicit_values(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(cli.watch_commands, "watch_task", watch)
    monkeypatch.setattr(cli.log_commands, "follow_logs", follow)

    assert cli.main([*base, "task", "show", task.task_id, "--watch"]) == 0
    assert (
        cli.main(
            [
                *base,
...
def test_continuous_cli_translates_interruptions_without_traceback(
    ...
    monkeypatch.setattr(cli.sys, "stdout", terminal)
    monkeypatch.setattr(
        cli.watch_commands, "watch_task", lambda *_args, **_kwargs: (_ for _ in ()).throw(KeyboardInterrupt)
    )
    assert cli.main([*base, "task", "show", task.task_id, "--watch"]) == 130
    assert terminal.getvalue() == "\x1b[0m\n"

    monkeypatch.setattr(
        cli.log_commands, "follow_logs", lambda *_args, **_kwargs: (_ for _ in ()).throw(KeyboardInterrupt)
    )
    assert cli.main([*base, "task", "logs", task.task_id, "--follow"]) == 130


def test_continuous_help_explains_refresh_tail_and_retry_contract(capsys) -> None:
    with pytest.raises(SystemExit) as show_exit:
        cli.main(["task", "show", "--help"])
