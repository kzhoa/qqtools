...
| qexp agent exit diagnostics, bounded evidence retention, and private log rotation | [diagnostic model Unit](unit/qexp/test_agent_diagnostics.py), [real-process diagnostics Integration](integration/qexp/test_agent_exit_diagnostics.py) | unit / qexp-integration | covered |
...
| qexp history-independent Group service activation, locator fairness, bounded residency, quiescence, and degraded repair | [protocol Integration](integration/qexp/test_group_service_quiescence.py), [scale qualification](integration/qexp/test_group_service_scale.py) | qexp-integration / retained stress qualification | covered |
| qexp resumable full audit, durable maintenance outbox, descriptor fairness, and capacity-independent resident progress | [descriptor and CLI Integration](integration/qexp/test_resumable_index_maintenance.py), [resident service Integration](integration/qexp/test_machine_dispatch.py), [availability recovery Integration](integration/qexp/test_availability_transitions.py) | qexp-integration / source preflight | covered |
