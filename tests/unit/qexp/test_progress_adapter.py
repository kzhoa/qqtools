...
@pytest.mark.parametrize("mode,rich,tqdm,expected", [
    ("rich", True, True, "rich"), ("rich", False, True, "tqdm"),
    ("tqdm", True, True, "tqdm"), ("plain", True, True, "plain"),
])
...
def test_nonzero_rank_does_not_construct_writer(monkeypatch, rank):
    monkeypatch.setenv("QEXP_PROGRESS_PATH", "/unused")
    monkeypatch.setattr(qexp_progress, "Reporter", lambda *_: pytest.fail("nonzero rank created writer"))
    bindings = Bindings()
...
def test_peer_fact_adapter_and_optimizer_step_semantics(monkeypatch):
    sink = Sink()
    monkeypatch.setenv("QEXP_PROGRESS_PATH", "/unused")
    monkeypatch.setattr(qexp_progress, "Reporter", lambda *_: sink)
    bindings = Bindings()
    ...
    assert len(sink.updates) == 1  # Do not count micro-batches as optimizer steps.
    ...
    for stage, expected in (("val", "validation"), ("test", "test")):
        fact = SimpleNamespace(stage=SimpleNamespace(value=stage), batch_index=9,
                               total_batches=100, global_step=7340)
...
def test_epoch_mode_unknown_total():
    sink = Sink()
    observer = qexp_progress.QexpProgressObserver(SimpleNamespace(max_steps=None), sink)
    observer.on_train(SimpleNamespace(global_step=10, epoch=2))
    assert sink.updates[-1]["total"] is None


def test_failed_reporter_setup_is_optional(monkeypatch):
    def broken(*_):
        raise OSError("unavailable")
    monkeypatch.setenv("QEXP_PROGRESS_PATH", "/unused")
    monkeypatch.setattr(qexp_progress, "Reporter", broken)
    assert qexp_progress.bind_qexp_progress(Bindings(), SimpleNamespace(rank=0)) is None
