...
def test_small_assignments_cover_labelled_fixed_capacity_partitions(group_count, slots):
    ...
    np.testing.assert_array_equal(
        np.sort(assignments.reshape(expected, total), axis=1), np.broadcast_to(np.arange(total), (expected, total))
    )
...
def test_repair_escapes_certified_two_batch_local_optimum():
    ...
    plan = balance._plan_rank_batches(
        costs, batch_size=3, world_size=2, seed=7, strategy="lpt_best", should_shuffle=False
    )
...
def test_repair_has_fixed_window_and_global_score_budgets(monkeypatch, should_certify_peak):
    ...
    repaired = balance._repair_rank_batches(
        ...
        2,
        7,
        peak if should_certify_peak else 0,
    )
    budget = balance._LPT_SECONDARY_REPAIR_WINDOWS if should_certify_peak else balance._LPT_REPAIR_WINDOWS
...
def test_repair_rejects_bad_global_candidate(monkeypatch):
    ...
    monkeypatch.setattr(
        balance, "_rank_batch_repair_candidates", lambda costs, rows, window: [np.sort(rows.ravel()).reshape(3, 4)]
    )
    repaired = balance._repair_rank_batches(costs, batches, 1, 7, 0)
    np.testing.assert_array_equal(repaired, batches)


@pytest.mark.parametrize("rows", [np.arange(6).reshape(6, 1), np.arange(6).reshape(1, 6), np.empty((0, 3), dtype=int)])
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [0, 0, 0, 0, 0, 100],
        [1e308, 1e308, 1, 1, 1, 1],
        [1e-320, 5e-324, 1e-323, 0, 0, 0],
        [0.1, 0.2, 0.3, 1e100, 1e-100, 0],
    ],
)
...
def test_repair_preserves_incumbent_occurrences_and_determinism(batch_size, world_size, seed):
    ...
    np.testing.assert_array_equal(
        repaired,
        balance._repair_rank_batches(costs, batches, world_size, seed, 0),
    )
