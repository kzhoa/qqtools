...
def test_step_refinement_never_worsens_metrics_or_mutates_input(
    ...
    actual = balance._optimize_step_batches(
        ...
        world_size,
        seed,
        should_use_best=should_use_best,
    )
...
def test_restores_step_score_without_changing_peak_or_p99():
    costs = np.array([4, 65, 24, 67, 34, 3, 55, 18, 90, 58, 44, 56.0])
...
@pytest.mark.parametrize(
    "batches,world_size",
    [(np.empty((0, 2), dtype=int), 2), (np.arange(8).reshape(4, 2), 1), (np.arange(8).reshape(8, 1), 2)],
)
def test_skips_unnecessary_search(monkeypatch, batches, world_size):
    ...
    assert balance._optimize_step_batches(np.arange(8.0), batches, world_size, 7) is batches


def test_rejects_regressing_candidate_from_either_search(monkeypatch):
    costs = np.array([9.0, 7, 2, 1])
