...
from qqtools.data.qbalance import _best_rank_batch_swaps as _best_pair_swaps
...
def test_two_pointer_matches_exhaustive_pair_gap(batch_size, distribution, seed):
    ...
    values = (
        rng.uniform(0, 100, (14, batch_size))
...
def test_finds_exchange_missed_by_four_positions():
    high = np.array([[36712, 113, 76, 55, 42, 32, 25, 20, 16, 12, 10, 7, 5, 4, 2, 0.0]])
    low = np.array([[235, 175, 104, 71, 55, 39, 32, 25, 19, 15, 12, 9, 7, 5, 4, 1.0]])
...
def test_integrated_fast_preserves_occurrences_and_improves_layered(seed, batch_size):
    ...
    for strategy in ("lpt_fast", "lpt", "lpt_best"):
        plan = _plan_rank_batches(
            costs, batch_size=batch_size, world_size=4, seed=seed, strategy=strategy, should_shuffle=False
        )
...
def test_odd_middle_batch_is_not_touched():
    costs = np.array([100, 10, 7, 1, 50, 20, 9, 2, 40, 30, 8, 3.0])
