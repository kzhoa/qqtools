"""v2 scheduler — dispatch queued tasks and reconcile running ones.

Integrates with:
- v1 tracker (GPU allocation, probing)
- v2 executor (tmux window + runner launch)
- v2 storage (CAS state transitions)
"""
...
class Scheduler:
    """Full v2 scheduler with GPU allocation and tmux execution."""

    tracker: Any  # qExpTracker-compatible
    ...
    def run_dispatch_cycle(self, cfg: RootConfig) -> list[str]:
        ...
        for task_id in queued_ids:
            ...
            try:
                window_id = self.executor.launch_in_window(cfg, task_id)
                log.info(
                    "Launched task %s in window %s with GPUs %s.",
                    task_id, window_id, assigned,
                )
...
# ---------------------------------------------------------------------------
# Standalone functions for backward compatibility with agent.py
# ---------------------------------------------------------------------------
