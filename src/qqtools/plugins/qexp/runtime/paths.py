...
def shared_paths(root: Path) -> dict[str, Path]:
    return {
        ...
        "ready_primary": root / "indexes" / "ready" / "primary",
        "ready_group_members": root / "indexes" / "ready" / "group-members",
        "ready_group_member_groups": root / "indexes" / "ready" / "group-members" / "groups",
...
def local_paths(root: Path) -> dict[str, Path]:
    return {
        ...
        "cpu_policy": root / "cpu-lane" / "policy.json",
        "cpu_provisional": root / "cpu-reservations" / "provisional",
...
def machine_runtime_paths(root: Path) -> dict[str, Path]:
    ...
    return {
        ...
        "diagnostic_lock": root / "locks" / "agent-diagnostics.lock",
        "diagnostic_instances": root / "diagnostics" / "instances",
        "diagnostic_metadata": root / "diagnostics" / "metadata.json",
        "diagnostic_last_exit": root / "diagnostics" / "last-exit.json",
